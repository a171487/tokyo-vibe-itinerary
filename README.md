...
<style id="trip-route-map-style">
  #tripRouteMap { width: 100%; height: 320px; border-radius: 14px; overflow: hidden; }
  .trip-route-wrap { margin-top: 12px; }
  .trip-daybar { display:flex; gap:8px; overflow-x:auto; padding: 6px 0 10px; -webkit-overflow-scrolling:touch; }
  .trip-daybar button { flex:0 0 auto; padding:8px 12px; border-radius:999px; border:1px solid rgba(255,255,255,.14); background: rgba(255,255,255,.06); color:#fff; font-weight:600; }
  ...
  .trip-map-actions a { flex:1; text-align:center; text-decoration:none; padding:10px 12px; border-radius:12px; border:1px solid rgba(255,255,255,.14); background: rgba(255,255,255,.06); color:#fff; font-weight:600; }
  .trip-map-note { font-size:12px; opacity:.78; margin-top:8px; line-height:1.35; }
  .leaflet-popup-content { margin: 10px 12px; }
  .seg-label { font-size:12px; font-weight:700; }
  .seg-sub { font-size:11px; opacity:.8; margin-top:2px; }
</style>
</head>
...
<!-- ===== Trip Route Map (Polyline + Distance + Transit ETA) ===== -->
<script>
(function(){
  ...
  wrap.className = 'trip-route-wrap';
  wrap.innerHTML = `
    <div class="trip-daybar" id="tripDayBar"></div>
    <div id="tripRouteMap"></div>
    <div class="trip-map-actions">
      <a id="openDayRouteGmaps" href="#" target="_blank" rel="noopener">在 Google Maps 開啟當日路線</a>
      <a id="recenterRouteMap" href="javascript:void(0)">地圖置中</a>
    </div>
    <div class="trip-map-note">
      地圖為「相對位置/距離」與「大眾運輸估時」視覺化：城市內短距離多以步行/地鐵為主；跨城以京成線/Skyliner 為主。
      （估時為常見區間值，實際仍以當天班次/路況為準。）
    </div>
  `;
  tripSection.prepend(wrap);

  const ROUTES = {"2024-12-26": {"stops": [{"name": "成田機場 NRT T2", "query": "Narita Airport Terminal 2 Japan"}, {"name": "上野車站", "query": "Ueno Station Tokyo Japan"}, {"name": "阿美橫丁", "query": "Ameyoko Shopping Street Tokyo"}, {"name": "GU 御徒町店", "query": "ジーユー 御徒町店 Tokyo"}, {"name": "3 COINS 上野店", "query": "3COINS Ueno Tokyo"}, {"name": "OS Drug 上野店", "query": "OS Drug Ueno Tokyo"}, {"name": "無印良品 上野丸井店", "query": "無印良品 上野丸井店 Tokyo"}], "legs": [{"mode": "Skyliner（大眾運輸）", "min": 41, "note": "NRT T2→京成上野"}, {"mode": "步行", "min": 5, "note": "上野站→阿美橫丁"}, {"mode": "步行", "min": 6, "note": "阿美橫丁→御徒町"}, {"mode": "步行", "min": 10, "note": "GU→3COINS"}, {"mode": "步行", "min": 3, "note": "3COINS→OS Drug"}, {"mode": "步行", "min": 7, "note": "OS Drug→無印良品（上野丸井）"}]}, "2024-12-27": {"stops": [{"name": "Tricolore Coffee（銀座）", "query": "Tricolore Ginza 5-9-17 Ginza Tokyo"}, {"name": "3 COINS 原宿旗艦店", "query": "3COINS 原宿旗艦店 Tokyo"}, {"name": "酒と焼肉 ニュートミー（惠比壽）", "query": "酒と焼肉 ニュートミー 恵比寿 Tokyo"}, {"name": "惠比壽花園廣場", "query": "Yebisu Garden Place Tokyo"}, {"name": "目黑川（中目黑）", "query": "Meguro River Naka-meguro Tokyo"}], "legs": [{"mode": "地鐵", "min": 18, "note": "銀座→原宿"}, {"mode": "地鐵", "min": 15, "note": "原宿→惠比壽"}, {"mode": "步行", "min": 6, "note": "惠比壽站→餐廳"}, {"mode": "步行", "min": 10, "note": "餐廳→惠比壽花園廣場"}, {"mode": "地鐵", "min": 8, "note": "惠比壽→中目黑（目黑川）"}]}, "2024-12-28": {"stops": [{"name": "東京車站（丸之內南口）", "query": "Tokyo Station Marunouchi South Exit"}, {"name": "新倉山淺間公園", "query": "Arakurayama Sengen Park"}, {"name": "日川時計店", "query": "Hikawa Clock Shop Fujiyoshida"}, {"name": "忍野八海", "query": "Oshino Hakkai"}, {"name": "大石公園（河口湖）", "query": "Oishi Park Lake Kawaguchi"}], "legs": [{"mode": "巴士/包車", "min": 150, "note": "東京→新倉山（含路況）"}, {"mode": "車程", "min": 10, "note": "新倉山→日川時計店"}, {"mode": "車程", "min": 20, "note": "日川時計店→忍野八海"}, {"mode": "車程", "min": 35, "note": "忍野八海→大石公園"}]}, "2024-12-29": {"stops": [{"name": "東京車站", "query": "Tokyo Station"}, {"name": "Sukiyaki Juni Ten（丸之內）", "query": "すき焼き 十二天 丸の内2-6-1 ブリックスクエア"}, {"name": "KITTE 百貨", "query": "KITTE Marunouchi Tokyo"}, {"name": "UNIQLO 銀座旗艦店", "query": "UNIQLO Ginza Flagship Store"}, {"name": "MARLOWE", "query": "MARLOWE pudding Tokyo Marunouchi"}, {"name": "芝公園", "query": "Shiba Park Tokyo Tower"}, {"name": "Stellar Garden Sky Bar", "query": "Stellar Garden Sky Bar & Dining Tokyo"}, {"name": "麻布台 Hills 森JP Tower", "query": "Azabudai Hills Mori JP Tower"}], "legs": [{"mode": "步行", "min": 6, "note": "東京站→餐廳"}, {"mode": "步行", "min": 6, "note": "餐廳→KITTE"}, {"mode": "地鐵", "min": 12, "note": "丸之內→銀座"}, {"mode": "步行", "min": 6, "note": "UNIQLO→MARLOWE"}, {"mode": "地鐵", "min": 18, "note": "銀座→芝公園"}, {"mode": "地鐵+步行", "min": 15, "note": "芝公園→酒吧"}, {"mode": "地鐵/步行", "min": 12, "note": "酒吧→麻布台 Hills"}]}, "2024-12-30": {"stops": [{"name": "NEWoMan TAKANAWA", "query": "NEWoMan TAKANAWA Tokyo"}, {"name": "二木菓子 秋葉原本店", "query": "Niki no Kashi Akihabara"}], "legs": [{"mode": "JR", "min": 25, "note": "高輪→秋葉原"}]}, "2024-12-31": {"stops": [{"name": "上野車站", "query": "Ueno Station Tokyo"}, {"name": "成田山新勝寺", "query": "Naritasan Shinshoji Temple"}, {"name": "成田山表參道", "query": "Naritasan Omotesando Narita"}, {"name": "成田夢牧場 門前店", "query": "Narita Yume Bokujo Monzen"}, {"name": "成田機場 NRT T2", "query": "Narita Airport Terminal 2 Japan"}], "legs": [{"mode": "大眾運輸（京成線）", "min": 89, "note": "上野→成田（含轉乘/步行）"}, {"mode": "步行", "min": 12, "note": "成田站→新勝寺/參道"}, {"mode": "步行", "min": 6, "note": "新勝寺→表參道"}, {"mode": "步行", "min": 10, "note": "表參道→夢牧場門前店"}, {"mode": "巴士/電車", "min": 25, "note": "成田市區→機場"}]}};
  ...
  const bar = document.getElementById('tripDayBar');

  let map, group, line, lastBounds;
  function ensureMap(){
    if(map) return;
    map = L.map('tripRouteMap', { zoomControl:true });
    ...
    document.getElementById('recenterRouteMap').addEventListener('click', ()=>{
  ...
  function buildGmapsUrl(dayKey){
    const r = ROUTES[dayKey];
    if(!r) return 'https://www.google.com/maps';
    const pts = r.stops.map(s=>s.name);
    if(pts.length<2) return 'https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(pts[0]||'Tokyo');
    const origin=pts[0], destination=pts[pts.length-1], waypoints=pts.slice(1,-1).join('|');
  ...
  let activeDay = '2024-12-26';

  async function renderDay(dayKey){
    activeDay = dayKey;
    [...bar.querySelectorAll('button')].forEach(b=>b.classList.toggle('active', b.dataset.key===dayKey));
    ...
    for(const s of r.stops){
      const q = s.query || (s.name + ' Tokyo Japan');
      const c = await geocode(q);
      coords.push(c ? {...c, name:s.name} : null);
    }
    ...
    coords.forEach((c, idx)=>{
      ...
      latlngs.push(ll);
      const m = L.marker(ll).addTo(group);
      const leg = r.legs[idx] ? r.legs[idx] : null; // next leg info
      const popup = leg
        ? `<div><b>${idx+1}. ${c.name}</b><div class="seg-sub">下一段：${leg.mode} 約 ${leg.min} 分</div></div>`
        : `<div><b>${idx+1}. ${c.name}</b></div>`;
      m.bindPopup(popup);
    });

    if(latlngs.length>=2){
      line = L.polyline(latlngs, {weight:4, opacity:0.85}).addTo(group);

      for(let i=0;i<coords.length-1;i++){
        const a=coords[i], b=coords[i+1];
        if(!a || !b) continue;
        const dkm = haversine(a,b);
        const leg = r.legs[i] || null;
        const label = leg
          ? `<div class="seg-label">${leg.mode} · 約 ${leg.min} 分</div><div class="seg-sub">距離約 ${dkm.toFixed(1)} km</div>`
          : `<div class="seg-label">距離約 ${dkm.toFixed(1)} km</div>`;
        const mid = [(a.lat+b.lat)/2, (a.lon+b.lon)/2];
        L.marker(mid, {
          icon: L.divIcon({
            className:'',
            html:`<div style="background:rgba(0,0,0,.55);color:#fff;padding:6px 8px;border-radius:10px;border:1px solid rgba(255,255,255,.22);">${label}</div>`
          })
        }).addTo(group);
      }
    ...
    document.getElementById('openDayRouteGmaps').href = buildGmapsUrl(dayKey);
  }
